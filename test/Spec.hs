...
test_typeCheckExpr =
  "expression type checking tests"
    ~: TestList
      [ runReaderT (typeCheckExpr False (Lit (BooleanLiteral True))) initialTSTypeEnv
          ~?= Right TBoolean,
        runReaderT (typeCheckExpr False (Var (Name "x"))) initialTSTypeEnv {localEnv = Map.singleton "x" TNumber}
          ~?= Right TNumber
      ]
